...
model = dict(
    type='TR3DFF3DDetector_scannet',
    voxel_size=voxel_size,
    backbone=dict(type='MinkFFResNet', in_channels=3, max_channels=128, depth=34, norm='batch'),
    neck=dict(
        type='TR3DNeck',
...
dataset_type = 'ScanNetDataset_fuse'
...
train_pipeline = [
    ...
    dict(type='SPPointSample', num_points=.33),
    dict(
        type='RandomFlip3D',
        sync_2d=False,
    ...
    dict(type='Collect3D', keys=['points', 'fused_points', 'gt_bboxes_3d', 'gt_labels_3d'])
]
test_pipeline = [
    dict(
        type='LoadPointsFromFile',
    ...
    dict(
        ...
        transforms=[
            ...
            dict(
                ...
                with_label=False),
            dict(type='Collect3D', keys=['points','fused_points'])
        ])
]
data = dict(
    samples_per_gpu=4,
