...
class IIFNet3DDetector_fast(Base3DDetector):
    ...
    def _f_v2(self, x, img_features, img_metas, img_shape):
        points = x.decomposed_coordinates #坐标
        for i in range(len(points)):
            points[i] = points[i] * self.voxel_size
        projected_features = []
    ...
    def extract_feats(self, points, superpoints,fused_points,img_metas,ori_points=None):
        """Extract features from points.

        Args:
            points (list[Tensor]): Raw point clouds.
            superpoints (list[Tensor]): Superpoints of each sample.
            fused_points (list[Tensor]): Fused points of each sample.
            ori_points (list[Tensor]): Original points of each sample.
        ...
        if ori_points is not None:
            coord_2d, feats_2d = ME.utils.batch_sparse_collate(
                [(p[:, :3] / self.voxel_size, fused_feat) for p, fused_feat in zip(ori_points, fused_points)],
                device=points[0].device)
        else:
            coord_2d, feats_2d = ME.utils.batch_sparse_collate(
                [(p[:, :3] / self.voxel_size, fused_feat) for p, fused_feat in zip(points, fused_points)],
                device=points[0].device)
        
   
        
        feat_2d = ME.SparseTensor(coordinates=coord_2d, features=feats_2d)
        ...
        return x,feat_2d,decode_res

    
    
    
    def forward_train(self, points, superpoints, fused_points,
        ...
        """
       #传gt
        x,feat_2d,decode_res = self.extract_feats(points,superpoints,fused_points,img_metas) #feat_2d -sparsetensor,
        ...
        decode_out = [None, decode_res, feat_2d, x['decode_out']]
        ...
        for i in range(len(gt_bboxes_3d)):
            bbox = torch.cat((gt_bboxes_3d[i].gravity_center, gt_bboxes_3d[i].tensor[:, 3:]), dim=1)#转化为正常gt
            ...
            # bbox[...,6] *= -1 
            gt_bboxes.append(bbox) #就是文件加载的bbox
        
        stage_two_dict['gt_bboxes_3d'] = gt_bboxes #分批次的bbox tensor
        ...
        # loss,roi_loss = self.roi_head.forward_train(stage_tow_dict) #ori losses
    ...
    def forward_test(self, points,superpoints,fused_points,img_metas, img=None, **kwargs):
        """
        Args:
        ...
        if num_augs == 1:
            ...
            return self.simple_test(points[0], superpoints[0],fused_points[0],img_metas[0], img[0], **kwargs)
        else:
            return self.aug_test(points, superpoints, fused_points, img_metas, img, **kwargs)

    def simple_test(self, points, superpoints, fused_points, img_metas, img, *args, **kwargs):
