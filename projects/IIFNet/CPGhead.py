...
class CPGHead(BaseModule):
    def __init__(self,
                 ...
                 test_cfg=None,
                 init_cfg=None):
        super(CPGHead, self).__init__(init_cfg)
        self.pts_threshold = pts_threshold
        self.center_type = center_type

        # sample rois######
        self.roi_fg_ratio = roi_fg_ratio
    ...
    def forward_test(self, x, img_metas):
        ...
        """
        center_preds, bbox_preds, cls_preds, points, vote_offsets, vote_voxel_points, orgin_superpoints = self(x)
        return self._get_bboxes_test(center_preds, bbox_preds, cls_preds, points,
                                img_metas)


    @torch.no_grad()
    def _get_roi_single_targets(self, points, bbox_preds, cls_preds, gt_bboxes, gt_labels):
        ...
        # condition 1: point inside box   boxes(0.5,0.5,0) to (0.5,0.5,0.5)
        ...
        #cost = reg_cost #only consider reg cost
        ...
        #                          torch.ones_like(centerness) * -1) #将不在任何边界框内的点的中心性值设置为 -1
        # centerness = torch.where(topk_condition, centerness,
        #                          torch.ones_like(centerness) * -1) #将不符合 topk_condition 的点的中心性值也设置为 -1
        ...
        code_size = 7
        rois =  boxes.new_zeros(self.roi_per_image, code_size)
        fg_rois_per_image = int(np.round(self.roi_fg_ratio * self.roi_per_image))
        #select rois for two-stage fg+bg
        fg_inds = (cls_targets >=0).nonzero().view(-1)
        bg_inds = (cls_targets< 0).nonzero().view(-1)
        ...
        if fg_num_rois > 0 and bg_num_rois > 0:
            ...
            rand_num = torch.from_numpy(np.random.permutation(fg_num_rois)).type_as(cls_targets).long()
            fg_inds = fg_inds[rand_num[:fg_rois_per_this_image]]
            
            bg_rois_per_this_image = self.roi_per_image - fg_rois_per_this_image
            # sampling bg
            if bg_num_rois >= bg_rois_per_this_image:
                bg_rand_inds = torch.randperm(bg_num_rois, device=cls_targets.device)[:bg_rois_per_this_image]
            
            else:
            # 随机重复补齐
                extra_inds = torch.randint(0, bg_num_rois, (bg_rois_per_this_image - bg_num_rois,), device=cls_targets.device)
                bg_rand_inds = torch.cat([torch.randperm(bg_num_rois, device=cls_targets.device), extra_inds], dim=0)
            
            bg_inds = bg_inds[bg_rand_inds]

        elif fg_num_rois > 0 and bg_num_rois == 0:
            # sampling fg
            rand_num = np.floor(np.random.rand(self.roi_per_image) * fg_num_rois)
            rand_num = torch.from_numpy(rand_num).type_as(cls_targets).long()
            ...
            bg_rand_inds = torch.randint(0, bg_num_rois, (bg_rois_per_this_image,), device=cls_targets.device)
            bg_inds = bg_inds[bg_rand_inds]
            fg_inds = torch.tensor([], dtype=torch.long, device=cls_targets.device) 
        else:
        ...
        assert sampled_inds.max() < bbox_preds.shape[0], "Index out of bounds for bbox_preds"

        
        assert sampled_inds.numel() == self.roi_per_image, \
            f"Sampled {sampled_inds.numel()} RoIs, expected {self.roi_per_image}"
        
        rois = bbox_preds[sampled_inds]
        bbox_targets = bbox_targets[sampled_inds]
        cls_targets = cls_targets[sampled_inds]
        
        if self.code_size == 6:
            # for scannet, make it 7
            rois = torch.cat((rois, torch.zeros_like(rois[:, :1])), dim=1) # add yaw
        
        return rois,bbox_targets, cls_targets   
    
    
    def _get_rois_targets(self,points,bbox_preds,cls_preds,gt_bboxes_3d,gt_labels_3d,img_metas):
        
        rois, bbox_targets, cls_targets = [],[],[]
        
        for i in range(len(img_metas)):
            roi,bbox_target,cls_target = self._get_roi_single_targets( 
                points=[x[i] for x in points],
                bbox_preds=[x[i] for x in bbox_preds],
                cls_preds=[x[i] for x in cls_preds],
                gt_bboxes=gt_bboxes_3d[i],
                gt_labels=gt_labels_3d[i])
            rois.append(roi)
    ...
    def _loss_single(self, center_preds, bbox_preds, cls_preds, points,
                     vote_offsets, 
                     vote_voxel_points, orgin_superpoints, original_points,
    ...
    def _get_bboxes_test_single(self,center_preds, bbox_preds, cls_preds, points,
                           img_meta):
        ...
        """
        mlvl_bboxes, mlvl_scores,mlvl_labels,mlvl_center_pred = [], [],[],[]
        for center_pred, bbox_pred, cls_pred, point in zip(
            ...
            if len(scores) > self.test_cfg.nms_pre > 0:
                ...
                point = point[ids]
                labels = labels[ids]
                center_pred = center_pred[ids]

            bboxes = self._bbox_pred_to_bbox(point, bbox_pred)
            mlvl_bboxes.append(bboxes)
            mlvl_scores.append(scores)
            mlvl_labels.append(labels)
            mlvl_center_pred.append(center_pred)

        bboxes = torch.cat(mlvl_bboxes)
        scores = torch.cat(mlvl_scores)
        labels = torch.cat(mlvl_labels)
        center_preds =  torch.cat(mlvl_center_pred)
        # bboxes, scores, labels,center_preds = self._single_scene_multiclass_nms_v1(
        #     bboxes, scores,center_preds,img_meta)
        if self.code_size == 6:
            # for scannet, make it 7 add fake heading
            bboxes = torch.cat((bboxes, torch.zeros_like(bboxes[:, :1])), dim=1)
        return bboxes, scores, labels, center_preds

    def _get_bboxes_test(self, center_preds, bbox_preds, cls_preds, points, img_metas):
        results = []
        for i in range(len(img_metas)):
            result = self._get_bboxes_test_single(
                center_preds=[x[i] for x in center_preds],
                bbox_preds=[x[i] for x in bbox_preds],
                cls_preds=[x[i] for x in cls_preds],
                points=[x[i] for x in points],
    ...
    def _get_centerness(face_distances):
        ...
        """
        x_dims = face_distances[..., [0, 1]] #算点到边界框各个维度最小和最大距离的比例，并将这些比例相乘，来衡量点相对于边界框的中心性
    ...
    def _get_targets(self, original_points, points, center_preds, bbox_preds, cls_preds, gt_bboxes, gt_labels):
        ...
        cls_cost = self.cls_cost(cls_preds, gt_labels)
        cost = reg_cost + cls_cost
        #cost = reg_cost #only consider reg costs
        cost = torch.where(inside_box_condition, cost, float_max) 
        top_cost = torch.topk(
            cost,
            min(self.pts_threshold + 1, len(cost)), 
        ...
        if self.with_yaw:
            ...
            for i in torch.unique(pts_instance_mask): 
                ...
                if pts_semantic_mask[indices[0]] < self.num_classes: 
                    ...
                    instance_center[i] = gt_bboxes[:, :3][match_gt_id].to(center.device) #更新 instance_center 为匹配到的边界框中心
                else:
            ...
            idx = knn(knn_number, scene_points[None, :, :3].contiguous(), voxel_points[None, ::])[0].long() #KNN找到 voxel_points 中每个点在 scene_points 中的最近邻居索引
    ...
    def _single_scene_multiclass_nms_v1(self, bboxes, scores, centerness,img_meta):
        ...
        nms_bboxes, nms_scores, nms_labels,nms_centerness = [], [], [],[]
        for i in range(n_classes):
            ...
            class_centerness = centerness[ids]
            ...
            nms_centerness.append(class_centerness[nms_ids])

        if len(nms_bboxes):
            nms_bboxes = torch.cat(nms_bboxes, dim=0)
            nms_scores = torch.cat(nms_scores, dim=0)
            nms_labels = torch.cat(nms_labels, dim=0)
            nms_centerness = torch.cat(nms_centerness,dim=0)
        else:
            nms_bboxes = bboxes.new_zeros((0, bboxes.shape[1]))
            nms_scores = bboxes.new_zeros((0, ))
            nms_labels = bboxes.new_zeros((0, ))
            nms_centerness = bboxes.new_zeros((0, ))
        ...
        #     box_dim=box_dim,
        #     with_yaw=with_yaw,
        #     origin=(.5, .5, .5)) #原点位于几何中心

        return nms_bboxes, nms_scores, nms_labels, nms_centerness
