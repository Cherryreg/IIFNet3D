...
from mmdet3d.datasets.pipelines.transforms_3d import GlobalAlignment
...
class SPPointSample(object):
    ...
    def __call__(self, results):
        ...
        if self.based_on_superpoints:
            ...
            points = results['points']
            # 保存未采样点
            # results['ori_points'] = points.clone()
            ...
            fused_points = results.get('fused_points', None)
            ...
            if fused_points is not None:
                fused_points = fused_points[choices]
                results['fused_points'] = fused_points
...
#added for fused points transforms 与点一致
@PIPELINES.register_module()
class GlobalAlignment_scannet(GlobalAlignment):
    """Global alignment for ScanNet dataset.
    """
