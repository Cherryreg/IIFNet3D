...
def main():
    ...
    # else:
    #     print(f"⚠️ {model.__class__.__name__} has no forward_dummy, using forward instead.")
    #     model.forward_dummy = model.forward  # 临时绑定
    ...
    # #     print(f"{param.shape} - {param.numel()/1e6:.2f} M")
    # print(f"Total Params: {total_params:.2f} M")
    
    total_params = 0
    print(f"{'Module Name':50} {'Shape':30} {'Params (M)':>12}")
    print("-" * 100)

    for name, param in model.named_parameters():
        num_params = param.numel() / 1e6
        total_params += num_params
        print(f"{name:50} {str(list(param.shape)):30} {num_params:12.4f}")

    print("-" * 100)
    print(f"{'Total Params:':>83} {total_params:12.4f} M")
